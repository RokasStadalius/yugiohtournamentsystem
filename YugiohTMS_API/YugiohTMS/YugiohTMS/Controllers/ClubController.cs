...
namespace YugiohTMS.Controllers
{
    ...
    public class ClubController : ControllerBase
    {
        ...
        public async Task<IActionResult> JoinClub(int clubId, [FromBody] JoinClubRequest request)
        {
            ...
            if (club == null)
            {
                return NotFound(new JoinResponseDto { Message = "Club not found" });
            }

            var user = await _context.User.FindAsync(request.UserId);
            if (user == null)
            {
                return BadRequest(new JoinResponseDto { Message = "Invalid User ID" });
            }
            ...
            {
                return Conflict(new JoinResponseDto { Message = "You are already a member of this club." });
            }
            ...
            return Ok(new JoinResponseDto { Message = "Successfully joined the club." });
        }
        ...
        public async Task<IActionResult> SendInvitation(int clubId, [FromBody] InvitationRequest request)
        {
            ...
            {
                return BadRequest(new JoinResponseDto{ Message = "Invalid club ID or user ID or current user ID." });
            }
            ...
            {
                return BadRequest(new JoinResponseDto { Message = "Invitations can only be sent for private clubs." });
            }
            ...
            {
                return Unauthorized(new JoinResponseDto { Message = "Only the club owner can send invitations." });
            }

            var userToInvite = await _context.User.FindAsync(request.UserIdToInvite);
            if (userToInvite == null)
            {
                return BadRequest(new JoinResponseDto { Message = "User to invite not found." });
            }
            ...
            if (existingInvitation != null)
            {
                if (existingInvitation.Status.ToLower() == "accepted")
                    return BadRequest(new JoinResponseDto{ Message = "User is already a member of this club." });
                if (existingInvitation.Status.ToLower() == "sent")
                    return BadRequest(new JoinResponseDto { Message = "Invitation already sent to this user." });
            ...
            return Ok(new JoinResponseDto{ Message = "Invitation sent successfully." });
        }
    }
}
