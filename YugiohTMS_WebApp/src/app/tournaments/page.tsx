...
import { ArrowRight, Trophy, Plus, Sword, Skull, CalendarDays, MapPin, Repeat } from "lucide-react";
...
export default function UserTournamentsPage() {
  ...
  const fetchUserTournaments = async (userId: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/Tournament/user/${userId}`
      );
  ...
  const handleCreateTournament = async () => {
    if (
      ...
      !newTournamentType.trim() ||
      !newTournamentDate ||
      !newTournamentLocation.trim()
    ) {
      toast.error("All tournament information must be filled in");
      return;
    }

    if (newTournamentType === "Swiss Stage" && (!numberOfRounds || parseInt(numberOfRounds) < 1)) {
      toast.error("Please enter a valid number of rounds for Swiss Stage");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/Tournament/create-tournament`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            numOfRounds: newTournamentType === "Swiss Stage" 
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[...Array(3)].map((_, i) => (
                <Skeleton
                  key={i}
                  className="h-48 bg-zinc-900 rounded-xl border-2 border-zinc-800 animate-pulse"
                />
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tournaments.map((tournament: any) => (
                <Card
                  ...
                  onClick={() =>
                    router.push(`/tournaments/${tournament.iD_Tournament}`)
                  }
                >
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="space-y-3 mt-4">
                      ...
                      {tournament.type === "Swiss Stage" && (
                        <div className="flex items-center">
                          <Repeat className="w-4 h-4 mr-2 text-red-500" />
                          <span className="text-sm text-zinc-400">
                            {tournament.numOfRounds} Rounds
                          </span>
                        </div>
                      )}
